...
// use middleware to parse json data in the request body otherwise req.body will be undefined 
...
app.post("/notes", (req, res) => {
  ...
  notes.push(note);
  res.json({ message: "Note created successfully", note:notes });
});
...
// patch to update existing note, mainly used to update a single field of a resource
app.patch("/notes/:id", (req, res) => {
  const noteId = req.params.id;
  const updatedNote = req.body;
   notes[noteId].title = updatedNote.title || notes[noteId].title;
  res.json({ message: "Note updated successfully", note: notes[noteId] });
});
